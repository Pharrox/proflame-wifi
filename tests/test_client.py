...
class ClientTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            client = ProflameClient('127.0.0.1', 3838)
    ...
    async def test_invalid_message(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            with self.assertLogs(client._logger, 'WARNING') as cm:
                await run_async(client.connect())
                await run_async(client.wait_for_active())
                await run_async(client._connection.send('INVALID'))
                await run_async(event.wait())
    ...
    async def test_invalid_json_type(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            with self.assertLogs(client._logger, 'WARNING') as cm:
                await run_async(client.connect())
                await run_async(client.wait_for_active())
                await run_async(client._connection.send(payload))
    ...
    async def test_callbacks(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_state(ApiAttrs.FAN_SPEED, 0))
            await run_async(handled.wait())
    ...
    async def test_keepalive(self):
        ...
        server = MockFireplace()
        async with serve(server.serve, '127.0.0.1', 3838):
            client = ProflameClient('127.0.0.1', 3838, ping_interval=0.5)
            client._connection.register_recv_handler(handler)
    ...
    async def test_set_state_wait(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            self.assertEqual(await run_async(client.set_state(ApiAttrs.FAN_SPEED, 3, True)), 3)
    ...
    async def test_set_state_wait_timeout(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.connect())
            await run_async(client.wait_for_active())
            self.assertFalse(await run_async(client.set_state('ignore', 1, True, wait_timeout=0.5)))
            await run_async(client.disconnect())
