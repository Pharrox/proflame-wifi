...
from .util import fmt_log, run_async
...
class ManagerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            client = ProflameManager('127.0.0.1', 3838)
    ...
    async def test_get_current_temperature_c(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.CURRENT_TEMPERATURE))
    ...
    async def test_get_fan_speed(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FAN_SPEED))
    ...
    async def test_set_fan_speed(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FAN_SPEED))
            self.assertEqual(client.fan_speed, MAX_FAN_SPEED)
            await run_async(client.set_fan_speed(3, True))
    ...
    async def test_set_fan_speed_underrun(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FAN_SPEED))
            self.assertEqual(client.fan_speed, 3)
            await run_async(client.set_fan_speed(MAX_FAN_SPEED + 1, True))
            self.assertEqual(client.fan_speed, MAX_FAN_SPEED)
            await run_async(client.set_fan_speed(MIN_FAN_SPEED - 1, True))
    ...
    async def test_get_flame_height(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FLAME_HEIGHT))
    ...
    async def test_set_flame_height(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FLAME_HEIGHT))
            self.assertEqual(client.flame_height, MAX_FLAME_HEIGHT)
            await run_async(client.set_flame_height(3, True))
    ...
    async def test_set_flame_height_bounds(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FLAME_HEIGHT))
            self.assertEqual(client.flame_height, 3)
            await run_async(client.set_flame_height(MAX_FLAME_HEIGHT + 1, True))
            self.assertEqual(client.flame_height, MAX_FLAME_HEIGHT)
            await run_async(client.set_flame_height(MIN_FLAME_HEIGHT - 1, True))
    ...
    async def test_set_flame_height_mode_takeover(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.connect())
            self.assertEqual(client.status, ClientStatus.CONNECTED)
            await run_async(client.wait_for_state_available(ApiAttrs.OPERATING_MODE))
            await run_async(client.wait_for_state_available(ApiAttrs.FLAME_HEIGHT))
            self.assertEqual(client.operating_mode, OperatingMode.THERMOSTAT)
            self.assertEqual(client.flame_height, MAX_FLAME_HEIGHT)
            await run_async(client.set_operating_mode(OperatingMode.SMART, True))
    ...
    async def test_get_light_brightness(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.LIGHT_BRIGHTNESS))
    ...
    async def test_set_light_brightness(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.LIGHT_BRIGHTNESS))
            self.assertEqual(client.light_brightness, MAX_LIGHT_BRIGHTNESS)
            await run_async(client.set_light_brightness(3, True))
    ...
    async def test_set_light_brightness_bounds(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.LIGHT_BRIGHTNESS))
            self.assertEqual(client.light_brightness, 3)
            await run_async(client.set_light_brightness(MAX_LIGHT_BRIGHTNESS + 1, True))
            self.assertEqual(client.light_brightness, MAX_LIGHT_BRIGHTNESS)
            await run_async(client.set_light_brightness(MIN_LIGHT_BRIGHTNESS - 1, True))
    ...
    async def test_set_operating_mode(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_operating_mode(OperatingMode.OFF, True))
            self.assertEqual(client.operating_mode, OperatingMode.OFF)

            await run_async(client.set_operating_mode(OperatingMode.MANUAL, True))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)

            await run_async(client.set_operating_mode(OperatingMode.THERMOSTAT, True))
            self.assertEqual(client.operating_mode, OperatingMode.THERMOSTAT)
    ...
    async def test_get_pilot_mode(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.PILOT_MODE))
    ...
    async def test_set_pilot_mode(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_pilot_mode(PilotMode.INTERMITENT, True))
            self.assertEqual(client.pilot_mode, PilotMode.INTERMITENT)

            await run_async(client.set_pilot_mode(PilotMode.CONTINUOUS, True))
            self.assertEqual(client.pilot_mode, PilotMode.CONTINUOUS)
    ...
    async def test_get_preset(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_operating_mode(OperatingMode.MANUAL, True))
            await run_async(client.set_flame_height(0, True))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)
            self.assertEqual(client.flame_height, 0)
            self.assertEqual(client.preset, Preset.OFF)

            await run_async(client.set_flame_height(6, True))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)
    ...
    async def test_set_preset(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_operating_mode(OperatingMode.OFF, True))
            await run_async(client.set_preset(Preset.OFF, True))
            self.assertEqual(client.operating_mode, OperatingMode.OFF)
            self.assertEqual(client.preset, Preset.OFF)
            ...
            await run_async(client.set_operating_mode(OperatingMode.THERMOSTAT, True))
            await run_async(client.set_flame_height(6, True))
            await run_async(client.set_preset(Preset.MANUAL, True))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)
            self.assertEqual(client.flame_height, 6)
            self.assertEqual(client.preset, Preset.MANUAL)

            await run_async(client.set_flame_height(6, True))
            await run_async(client.set_preset(Preset.THERMOSTAT, True))
            self.assertEqual(client.operating_mode, OperatingMode.THERMOSTAT)
            self.assertEqual(client.flame_height, 6)
            self.assertEqual(client.preset, Preset.THERMOSTAT)

            await run_async(client.set_preset(Preset.SMART, True))
            self.assertEqual(client.operating_mode, OperatingMode.SMART)
            self.assertEqual(client.flame_height, 6)
            self.assertEqual(client.preset, Preset.SMART)
    ...
    async def test_set_preset_flame_restore(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.set_operating_mode(OperatingMode.THERMOSTAT, True))
            await run_async(client.set_flame_height(4, True))
            self.assertEqual(client.operating_mode, OperatingMode.THERMOSTAT)
    ...
    async def test_get_target_temperature_c(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.TARGET_TEMPERATURE))
            self.assertIsNone(client.target_temperature)
            await run_async(client.set_operating_mode(OperatingMode.THERMOSTAT, True))
    ...
    async def test_set_target_temperature_c(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.TARGET_TEMPERATURE))
            self.assertEqual(client.target_temperature, Temperature.celcius(25))
            await run_async(client.set_target_temperature(Temperature.celcius(26), True))
    ...
    async def test_set_target_temperature_f(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.TARGET_TEMPERATURE))
            self.assertEqual(client.target_temperature, Temperature.fahrenheit(75))
            await run_async(client.set_target_temperature(Temperature.fahrenheit(78), True))
    ...
    async def test_get_temperature_unit(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.TEMPERATURE_UNIT))
    ...
    async def test_turn_off(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.OPERATING_MODE))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)
            await run_async(client.turn_off(True))
            self.assertEqual(client.operating_mode, OperatingMode.OFF)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)
    ...
    async def test_turn_on(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.OPERATING_MODE))
            self.assertEqual(client.operating_mode, OperatingMode.OFF)
            await run_async(client.turn_on(True))
            self.assertEqual(client.operating_mode, OperatingMode.MANUAL)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)
    ...
    async def test_callbacks(self):
        ...
        server = MockFireplace(**{ApiAttrs.OPERATING_MODE: OperatingMode.MANUAL})
        async with serve(server.serve, '127.0.0.1', 3838):
            client = ProflameManager('127.0.0.1', 3838, callbacks=[handler])
            await run_async(client.connect())
            self.assertEqual(client.status, ClientStatus.CONNECTED)
            await run_async(client.wait_for_state_available(ApiAttrs.OPERATING_MODE))
            await run_async(asyncio.wait_for(called.wait(), timeout=3))
    ...
    async def test_turn_off_fan(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.FAN_SPEED))
            self.assertEqual(client.fan_speed, MAX_FAN_SPEED)
            await run_async(client.turn_off_fan(True))
            self.assertEqual(client.fan_speed, 0)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)
    ...
    async def test_turn_off_light(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.wait_for_state_available(ApiAttrs.LIGHT_BRIGHTNESS))
            self.assertEqual(client.light_brightness, MAX_LIGHT_BRIGHTNESS)
            await run_async(client.turn_off_light(True))
            self.assertEqual(client.light_brightness, 0)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)
    ...
    async def test_turn_on_fan(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.turn_off_fan(True))
            self.assertEqual(client.fan_speed, 0)
            await run_async(client.turn_on_fan(True))
            self.assertEqual(client.fan_speed, 5)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)
    ...
    async def test_turn_on_light(self):
        ...
        async with serve(server.serve, '127.0.0.1', 3838):
            ...
            await run_async(client.turn_off_light(True))
            self.assertEqual(client.light_brightness, 0)
            await run_async(client.turn_on_light(True))
            self.assertEqual(client.light_brightness, 5)
            await run_async(client.disconnect())
            self.assertEqual(client.status, ClientStatus.DISCONNECTED)